...
{
    public partial class eRecipe : FormBase, Views.ERecipe.IERecipe
    {
        ...
        public void _ucRecipeEdit_Show()
        {
            ...
            try
            {
                if (eRecipeItem.eRecipe_CompensationTag == true) //kompensuojamas receptas
                    _ucRecipeEdit = new ucRecipeEdit(eRecipeItem, posdItem, PickedUpByRef);
                else //nekompensuojamas receptas
                    _ucRecipeEdit = new ucRecipeEdit_notComp(eRecipeItem, posdItem, PickedUpByRef);
        ...
        private async void DeletePosd(bool refresh = true)
        {
            if (!Session.RecipesOnly && posdItem != null)
            {
                ...
                Serilogger.GetLogger().Information($"[DeletePosd] RecipeNumber: {eRecipeItem?.eRecipe?.RecipeNumber}, " +
                                                   $"Recipe composition ID: {eRecipeItem?.eRecipe?.CompositionId}, " +
                                                   $"Pos Header ID: {posdItem.hid}, Pos Detail ID: {posdItem.id}, " +
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
                ...
                dic.Add("", "Visi");
                dic.Add("completed", "Išduoti");
                dic.Add("entered in error", "Panaikinti");
                ...
                dic = new Dictionary<string, string>();
                dic.Add("", "Visi");
                dic.Add("Signed", "Pasirašyti");
                dic.Add("final", "Nepasirašyti");
                dic.Add("entered in error", "Panaikinti");
                ...
                dic = new Dictionary<string, string>();
                dic.Add("", "Visi");
                dic.Add("final,preliminary", "Išduoti\\Paskirti");
                dic.Add("entered in error", "Panaikinti");
            ...
            if (SelectedRecipeItems?.Count == 1)//po vieną
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        btnDiffActiveSub.Enabled = false;
                        btnReserve.Enabled = false;
                        btnReserve.Text = "Rezervuoti";
                        btnSuspend.Enabled = false;
                        btnSuspend.Text = "Sustabdyti";
        ...
        {
            await PerformSell(false);
        }

        private async void btnDiffActiveSub_Click(object sender, EventArgs e)
        {
            await PerformSell(true);
        }

        private async Task PerformSell(bool partialDispense) 
        {
            ...
            Display2Tag display2Tag = new Display2Tag() { IsPartialDispense = false };
            var selectedProduct = Program.Display2.ExecuteFromRemote(eRecipeItem.eRecipe, ref display2Tag);
            if (selectedProduct == null)
                return;

            if (!await _presenterList.IsDiagnosisValidByNpakid7(selectedProduct.NpakId7) &&
        ...
        {
            DeletePosd();
            Serilogger.GetLogger().Information($"[SellAlert] RecipeNumber: {eRecipeItem?.eRecipe?.RecipeNumber}, " +
                                   $"Recipe composition ID: {eRecipeItem?.eRecipe?.CompositionId}" +
                                   $"Message: {msg}");
        ...
        private async void VaccineUserControl_LoadEvent(object sender, System.EventArgs e)
        {
            ...
            if (!(sender is VaccineSellUserControl)) return;
            await SellVaccine(async (barcodeModel) =>
            {
                ...
                using (var d = new Popups.wpf_dlg(wpf, "Prekės pasirinkimas"))
                {
                    ...
                    if (d.DialogResult == DialogResult.OK)
                    {
                        ...
                        {
                            (sender as VaccineSellUserControl).Close();
                        }
                        else
                        {

                            Items.posd posd = Program.Display1.PoshItem?.PosdItems.FirstOrDefault();
                            (sender as VaccineSellUserControl).FillVaccineData(posd, barcodeModel.SerialNumber);
                        }
                    }
                    else
                        (sender as VaccineSellUserControl).Close();
        ...
        private bool CanUnlock(RecipeDto recipe) 
        {
            ...
            if (!string.IsNullOrWhiteSpace(recipe.LockDepartment))
            {
                ...
                Serilogger.GetLogger().Information($"[Recipe Unlock Checking] RecipeNumber: {eRecipeItem?.eRecipe?.RecipeNumber}, " +
                                   $"Locked department ID: {lockedDepartmentId}" +
                                   $" Department ID which trying to unlock: {currentPharmacyId}"+
                                   $" Pharmacy KAS ID: {Session.SystemData.kas_client_id}");
