...
namespace POS_display
{
    public partial class service : Form
    {
        ...
        private static AutoCompleteStringCollection typenameCol = new AutoCompleteStringCollection();
        ...
        #region Callbacks
        private void dbSearchService(bool success, DataTable t)
        {
        ...
        private void dbAutocompleteTypeName(bool success, DataTable t)
        {
            // we are different thread!
            this.Invoke(new MethodInvoker(delegate
            {
                foreach (DataRow row in t.Rows)
                {
                    typenameCol.Add(Convert.ToString(row["typename"]));
                    allCol.Add(Convert.ToString(row["typename"]));
                }
            }));
        }
        #endregion
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
            ...
            test.Add("name", "paslaugos pavadinimą");
            test.Add("typename", "paslaugos tipą");
            test.Add("name:typename", "paslaugos pavadinimą, paslaugos tipą");
            ...
            if (nameCol.Count == 0 && typenameCol.Count == 0)
            {
                DB.POS.asyncAutocomplete("search_service", "name", dbAutocompleteName);
                DB.POS.asyncAutocomplete("search_service", "typename", dbAutocompleteTypeName);
            }
        ...
        private void service_Closing(object sender, FormClosingEventArgs e)
        {
        ...
        private void GridView_order()
        {
            ...
            gvFilter.Columns["name"].HeaderText = "Paslaugos pavadinimas";
            gvFilter.Columns["name"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
            gvFilter.Columns["typename"].DisplayIndex = 1;
            gvFilter.Columns["typename"].HeaderText = "Tipas";
            gvFilter.Columns["typename"].AutoSizeMode = DataGridViewAutoSizeColumnMode.DisplayedCells;
            //hide columns
        ...
        private void btnFilter_Click(object sender, EventArgs e)
        {
            ...
            DB.POS.asyncSearchService(((KeyValuePair<string, string>)ddFilter.SelectedItem).Key, tbFilter.Text, dbSearchService);
        }
        ...
        private void gvFilter_DoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex < 0)
                return;
            serviceName = gvFilter.Rows[e.RowIndex].Cells["name"].Value.ToString();
            serviceId = gvFilter.Rows[e.RowIndex].Cells["id"].Value.ToString();
        ...
        private void autocomplete_source()
        {
            ...
            if (((KeyValuePair<string, string>)ddFilter.SelectedItem).Key.Equals("typename"))
                tbFilter.AutoCompleteCustomSource = typenameCol;
            if (((KeyValuePair<string, string>)ddFilter.SelectedItem).Key.Equals("name:typename"))
